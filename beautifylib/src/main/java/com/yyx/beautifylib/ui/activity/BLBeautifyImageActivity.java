...
public class BLBeautifyImageActivity extends BLToolBarActivity implements View.OnClickListener {
    ...
    private static int TXT_NORMAL_COLOR = 0x50e4ebf2;
...
//    private List<String> mMergeList = new ArrayList<>();
    ...
    private TextView photo_picker_menu_item_title2;
    ...
    protected void customToolBarStyle() {
        ...
        photo_picker_menu_item_title2 = (TextView) findViewById(R.id.photo_picker_menu_item_title2);
        photo_picker_menu_item_title2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.v("more", "if (item.getItemId() == R.id.preview_menu)");
        ...
        protected BLResultParam doInBackground(List<Fragment>... params) {
            ...
            for (Fragment f : fragments) {
                if (f instanceof BLBeautifyFragment) {
                    ...
                    Log.v("more", "path: " + path);

                    if (path != null && !path.equals("")) {

                        Log.v("more", "if (path != null && !path.equals(\"\"))");

                        mergeList.add(path);
                        if (fragment.getTagGroupModelList().size() != 0) {
                            Log.v("more", "if (fragment.getTagGroupModelList().size() != 0)");
